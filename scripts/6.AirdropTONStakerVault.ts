...
async function main() {
  ...
  const Vault = await ethers.getContractFactory("AirdropTONStakerVault");
  const vaultContract = await Vault.deploy(docAddress);

  await vaultContract.deployed();

  console.log("AirdropTONStakerVault Address:", vaultContract.address);
  ...
  //전송 후 DOC 토큰 전송 필요
  //npx hardhat verify --contract contracts/AirdropTONStakerVault.sol:AirdropTONStakerVault a b --network rinkeby
}
