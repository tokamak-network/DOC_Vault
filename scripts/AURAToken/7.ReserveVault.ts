...
async function main() {
  ...
  console.log("ReserveVault Address:", vaultContract.address);
  console.log("setting start");

  const BASE_TEN = 10
  const decimals = 18

  const totalAllocatedAmount = 4000000   //4,000,000
  const totalBigAmount = BigNumber.from(totalAllocatedAmount).mul(BigNumber.from(BASE_TEN).pow(decimals))
  const totalClaimCounts = 8             //8회
  const startTime = 1707552000            //2024년 2월 10일 토요일 오후 5:00:00
