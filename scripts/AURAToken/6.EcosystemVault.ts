...
async function main() {
  ...
  const Vault = await ethers.getContractFactory("EcosystemVault");
  const vaultContract = await Vault.deploy(auraToken);

  await vaultContract.deployed();

  console.log("EcosystemVault Address:", vaultContract.address);
  console.log("setting start");

  const BASE_TEN = 10
  const decimals = 18

  const totalAllocatedAmount = 30000000   //30,000,000
