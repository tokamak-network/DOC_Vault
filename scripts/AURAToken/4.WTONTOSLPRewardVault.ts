...
async function main() {
  ...
  const Vault = await ethers.getContractFactory("WTONTOSLPRewardVault");
  const vaultContract = await Vault.deploy(auraToken);

  await vaultContract.deployed();

  console.log("WTONTOSLPRewardVault Address:", vaultContract.address);
  console.log("setting start");

  const BASE_TEN = 10
  const decimals = 18

  const totalAllocatedAmount = 2500000   //2,500,000
