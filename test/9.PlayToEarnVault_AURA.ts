...
describe("WTONTOSLPRewardVault", () => {
    ...
    let totalClaim = 16
    let monthlyReward = totalAmount/totalClaim
    ...
    before(async () => {
        ...
        const TreasuryVault = await ethers.getContractFactory("PlayToEarnVault");
    ...
    describe("PlayToEarnVault Test", () => {
        describe("setting", () => {
            it("check name, doc ", async function() {
                expect(await stakerVault.name()).to.equal("PlayToEarn");
        ...
        describe("claim", () =>{
            it("claim before time", async () => {
                await expect(stakerVault.connect(tokenOwner).claim(
                    account1.address
                )).to.be.revertedWith("DragonsVault: not started yet");
            })
            ...
            it("claim after startTime", async () => {
                ...
                expect(Number(tx)).to.equal(monthlyReward);
            })
            ...
            it("claim after 1month", async () => {    
                ...
                let tx = await docToken.balanceOf(account2.address)
                console.log(Number(tx))
                console.log(Number(monthlyReward*2))
    
                expect(Number(tx)).to.equal(Number(monthlyReward*2));
            })

            it("claim after 4month", async () => {    
                await ethers.provider.send('evm_setNextBlockTimestamp', [startTime + (periodTimesPerClaim*4)]);
                await ethers.provider.send('evm_mine');
                ...
                expect(Number(tx)).to.equal(Number(monthlyReward*5));
            })

            it("claim after 36month", async () => {    
                await ethers.provider.send('evm_setNextBlockTimestamp', [startTime + (periodTimesPerClaim*15)]);
                await ethers.provider.send('evm_mine');
                ...
                let tx = await docToken.balanceOf(account2.address)
                console.log(Number(tx))
                console.log(Number(monthlyReward*16))
    
                expect(Number(tx)).to.equal(Number(monthlyReward*16));
            })
